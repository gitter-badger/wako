...
export class TraktMoviesPlayedForm {
  static submit(period: string, page: number, limit: number, filters?: TraktMoviesFilterStructure) {
    const params: any = {
      extended: 'full',
      page: page,
      limit: limit,
    ...
    return TraktApiService.get<{ watchers: number; movie: TraktMovieDto }[]>(`/movies/played/${period}`, params).pipe(
      map(results => {
